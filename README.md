...
במידה ובחרתם להשתמש בשרתים המותקנים אצלכם מקומית ולא ב-Docker, בצעו את הצעדים הבאים:

1. **SQL Server:**
    * פתחו את קובץ הסקריפט `db-script.sql` (נמצא בתיקיית `infrastructure`) והריצו אותו בשרת שלכם. הסקריפט יצור את בסיס הנתונים `ShoppingListDB` ואת כל הטבלאות הנדרשות.
    * ודאו שפרטי ההתחברות (User/Password) ב-`appsettings.json` תואמים להגדרות השרת המקומי שלכם.

2. **Elasticsearch:**
    * ודאו שהשרת המקומי רץ בפורט `9200`. 
    * עם עליית שרת ה-Node, הוא ינסה ליצור את ה-Index והמפינג (Mapping) באופן אוטומטי.

**שימו לב:** הגדרות ה-Connection String בפרויקט מוגדרות כברירת מחדל ל-`localhost`. במידה והשרתים המקומיים שלכם יושבים בכתובת אחרת או משתמשים בפורטים שונים, יש לעדכן זאת בקבצי הקונפיגורציה.
